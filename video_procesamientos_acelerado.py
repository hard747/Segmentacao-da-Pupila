...
# Habilita o uso de OpenCL no OpenCV para aceleração da GPU
...
class PupilSegmenter:
    ...
    def refine_roi(self, escura_gray, brilhante_gray):
        # Acelera as operações de diferença e morfológicas com UMat
        escura_umat = cv2.UMat(escura_gray)
        brilhante_umat = cv2.UMat(brilhante_gray)
        
        diff_umat = cv2.subtract(brilhante_umat, escura_umat)
        diff_blurred_umat = cv2.GaussianBlur(diff_umat, (9, 9), 0)
        _, roi_mask_umat = cv2.threshold(diff_blurred_umat, 40, 255, cv2.THRESH_BINARY)

        # Processamento morfológico para refinar a ROI na GPU
        roi_mask_umat = cv2.morphologyEx(roi_mask_umat, cv2.MORPH_CLOSE, self.kernel_medium, iterations=2)
        roi_mask_umat = cv2.morphologyEx(roi_mask_umat, cv2.MORPH_OPEN, self.kernel_medium, iterations=2)
        roi_mask_umat = cv2.erode(roi_mask_umat, self.kernel_small, iterations=1)
        roi_mask_umat = cv2.dilate(roi_mask_umat, self.kernel_small, iterations=1)

        # Converte a máscara de volta para uma matriz NumPy para uso posterior
        roi_mask = roi_mask_umat.get()
        diff = diff_umat.get()
    ...
    def remove_glints_from_image(self, img_gray, glints_info):
        ...
        for glint in glints_info:
            ...
            cv2.circle(img_no_glints, center, radius, 0, -1)  # Círculo preto
    ...
    def segment_pupil_dark(self, escura_gray_no_glints, roi_mask, frame_original):
        ...
        try:
            # Acelera o processamento com UMat
            escura_umat = cv2.UMat(escura_gray_no_glints)
            roi_mask_umat = cv2.UMat(roi_mask)

            _, thresh_inv_umat = cv2.threshold(escura_umat, 50, 255, cv2.THRESH_BINARY_INV)
            thresh_inv_roi_umat = cv2.bitwise_and(thresh_inv_umat, thresh_inv_umat, mask=roi_mask_umat)

            opening_umat = cv2.morphologyEx(thresh_inv_roi_umat, cv2.MORPH_OPEN, self.kernel_small, iterations=2)
            sure_bg_umat = cv2.dilate(opening_umat, self.kernel_small, iterations=3)
            dist_transform_umat = cv2.distanceTransform(opening_umat, cv2.DIST_L2, 5)

            dist_transform = dist_transform_umat.get()
            dist_max = dist_transform.max()
            _, sure_fg_umat = cv2.threshold(dist_transform_umat, 0.5 * dist_max, 255, 0)
            
            sure_bg = sure_bg_umat.get()
            sure_fg = sure_fg_umat.get()
            ...
            if np.max(markers) > 1:
                # O watershed pode não ser acelerado via OpenCL, mas o resto do pipeline é
    ...
    def segment_pupil_bright(self, brilhante_gray_no_glints, roi_mask, frame_original):
        ...
        try:
            # Acelera o processamento com UMat
            brilhante_umat = cv2.UMat(brilhante_gray_no_glints)
            roi_mask_umat = cv2.UMat(roi_mask)
            roi_no_glints_umat = cv2.bitwise_and(brilhante_umat, brilhante_umat, mask=roi_mask_umat)

            min_val, max_val, _, _ = cv2.minMaxLoc(brilhante_gray_no_glints, mask=roi_mask)
            threshold_val = 60
            if max_val > 0:
                threshold_val = int(max_val * 0.5)
                threshold_val = max(50, min(200, threshold_val))

            _, thresh_umat = cv2.threshold(roi_no_glints_umat, threshold_val, 255, cv2.THRESH_BINARY)
            thresh_umat = cv2.morphologyEx(thresh_umat, cv2.MORPH_OPEN, self.kernel_small, iterations=1)
            thresh_umat = cv2.morphologyEx(thresh_umat, cv2.MORPH_CLOSE, self.kernel_small, iterations=1)

            opening_umat = cv2.morphologyEx(thresh_umat, cv2.MORPH_OPEN, self.kernel_small, iterations=2)
            sure_bg_umat = cv2.dilate(opening_umat, self.kernel_small, iterations=3)
            dist_transform_umat = cv2.distanceTransform(opening_umat, cv2.DIST_L2, 5)
            ...
            dist_max = dist_transform.max()
            if dist_max > 0:
                _, sure_fg_umat = cv2.threshold(dist_transform_umat, 0.4 * dist_max, 255, 0)
            else:
                sure_fg_umat = cv2.UMat(np.zeros_like(brilhante_gray_no_glints))
    ...
    def segment_pair(self, frame_escura, frame_brilhante, par_idx):
        ...
        # Converte para UMat para aceleração no próximo passo
...
def main():
    ...
    # Certifique-se de que o OpenCV está configurado corretamente com OpenCL
    if not cv2.ocl.haveOpenCL():
        print("Erro: OpenCL não está habilitado. Certifique-se de ter os drivers e bibliotecas OpenCL corretos.")
        return
